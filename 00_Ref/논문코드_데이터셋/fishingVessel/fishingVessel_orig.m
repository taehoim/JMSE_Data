function [xdot, U, M] = fishingVessel(x, tau_wave, L, B, T, tonnage)
% Compatible with MATLAB and GNU Octave (www.octave.org)
% [xdot, U, M] = fishingVessel(x, tau_wave, L, B, T, tonnage) returns the 
...
if isempty(vessel_params) || (nargin > 0 && nargin < 6)
    ...
    % Ensure positive GM
    if vessel_params.GM_T < 0.3
        vessel_params.GM_T = 0.3;
    end
